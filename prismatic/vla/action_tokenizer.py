...
class VQActionTokenizer(ActionTokenizer):
    ...
    def decode_token_ids_to_actions(self, action_token_ids: np.ndarray) -> np.ndarray:
        ...
        # --> (1 x A) --> (A,)
        ret_action = self.vq_vae.get_action_from_latent(latent)[0]

        # print(action_token_ids, action_token_ids.shape, ret_action.shape)
        ...
        # return ret_action[:, 0]
        return [ret_action[i] for i in range(len(ret_action[0]))]
