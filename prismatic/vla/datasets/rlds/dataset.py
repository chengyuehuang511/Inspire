...
def make_dataset_from_rlds(
    ...
    *,
    ...
    if True:
        ...
        # full_dataset = full_dataset.filter(lambda x: tf.strings.regex_full_match(x["task"]["language_instruction"][0], ".*black bowl.*"))
    ...
    # only keep black bowl in instruction
    # dataset = dataset.filter(lambda x: tf.strings.regex_full_match(x["task"]["language_instruction"][0], ".*black bowl.*"))
