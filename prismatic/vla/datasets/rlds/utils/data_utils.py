...
def get_dataset_statistics(
    ...
    dataset = dataset.traj_map(
        lambda traj: {
            ...
            "language_instruction": traj['task']['language_instruction'],
        }
    )
    ...
    for traj in tqdm(dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None):
        ...
        num_trajectories += 1
        instruction_counts[traj['language_instruction'][0].decode()] += 1
    
    for instruction, count in instruction_counts.items():
        overwatch.info(f"Instruction: {instruction} =>> Count: {count}")
