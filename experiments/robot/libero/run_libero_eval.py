...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "pretrained/minivla-libero90-prismatic/checkpoints/step-122500-epoch-55-loss=0.0743.pt"     # Pretrained checkpoint path
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        if task.language != 'put the black bowl in the top drawer of the cabinet':
            continue
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            while t < max_steps + cfg.num_steps_wait:
                try:
                    ...
                    if not info['is_grasped'] and action[-1] == 1:
                        not_grasp_but_close_count += 1
                    
                    if not_grasp_but_close_count >= 20:
                        action[-1] = -1
                        force_open_count += 1
                        if force_open_count >= 10:
                            not_grasp_but_close_count = 0
                            force_open_count = 0
